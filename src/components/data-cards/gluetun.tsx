...
import { FAKE_IP, FAKE_CITY, FAKE_COUNTRY, FAKE_ORGANIZATION } from "@/lib/anonymize";

export function Gluetun({ isAnonymized }: { isAnonymized: boolean }) {
            ...
            <CardContent>
                <div className="grid grid-cols-2 gap-2 h-full">
                    <div className="backdrop-filter backdrop-blur-lg bg-zinc-900/60 border border-white/10 shadow-lg p-2 rounded-md flex flex-col justify-between">
                        ...
                        <div className="text-white text-sm">{isLoading || error ? "..." : isAnonymized ? FAKE_IP : (data?.public_ip ?? "127.0.0.1")}</div>
                    </div>
                    <div className="backdrop-filter backdrop-blur-lg bg-zinc-900/60 border border-white/10 shadow-lg p-2 rounded-md flex flex-col justify-between">
                        <span className="text-zinc-400 text-xs">Location</span>
                        <div className="text-white text-sm">{isLoading || error ? "..." : isAnonymized ? `${FAKE_CITY}, ${FAKE_COUNTRY}` : `${data?.city}, ${data?.country}`}</div>
                    </div>
                    <div className="backdrop-filter backdrop-blur-lg bg-zinc-900/60 border border-white/10 shadow-lg p-2 rounded-md flex flex-col justify-between">
                        <span className="text-zinc-400 text-xs">Provider</span>
                        <div className="text-white text-sm">{isLoading || error ? "..." : isAnonymized ? FAKE_ORGANIZATION : (data?.organization ?? "Unknown")}</div>
                    </div>
                    <div className="backdrop-filter backdrop-blur-lg bg-zinc-900/60 border border-white/10 shadow-lg p-2 rounded-md flex flex-col justify-between">
